...
public:
    ...
    void (*set_aim)(void *, Quaternion look, bool KDCHOPMDKKE);
...
void game_sdk_t::init()
{
    ...
    this->GetLocalPlayer = (void *(*)(void *))getRealOffset(oxo("0x5919628"));
    ...
    this->WorldToViewpoint = (Vector3(*)(void*, Vector3, int))getRealOffset(oxo("0x8CFE6C0"));

    this->get_isVisible = (bool (*)(void *))getRealOffset(oxo("0x563B10C"));

    this->get_isLocalTeam = (bool (*)(void *))getRealOffset(oxo("0x591C5B4"));
    ...
    this->set_aim = (void (*)(void *, Quaternion, bool))getRealOffset(oxo("0x5636F80"));
    ...
    this->_getRightForeArmTF = (void *(*)(void *))getRealOffset(oxo("0x5636224"));
}


static void SwapWeapon(void *player, int POFFNNMOOBM, bool GDKLMFLNNGM) {
    void (*_SwapWeapon)(void *player, int POFFNNMOOBM, bool GDKLMFLNNGM) = (void (*)(void *, int, bool))getRealOffset(ENCRYPTOFFSET("0x56B4EB4"));
    ...
    static Vector3 Transform_INTERNAL_GetPosition(void *player) {
        ...
        void (*_Transform_INTERNAL_GetPosition)(void *transform, Vector3 * out) = (void (*)(void *, Vector3 *))getRealOffset(ENCRYPTOFFSET("0x8D6C250"));
        _Transform_INTERNAL_GetPosition(player, &out);
        return out;
    }

    static void Transform_INTERNAL_SetPosition(void *player, Vvector3 inn) {
        void (*_Transform_INTERNAL_SetPosition)(void *transform, Vvector3 *in) = (void (*)(void *, Vvector3 *))getRealOffset(ENCRYPTOFFSET("0x8D6C328"));
...
static Vector3 CameraMain(void *player) {
    ...
    void *tf = cam ? game_sdk->Component_GetTransform(cam) : nullptr;
...
public:
    static Vector3 Transform_GetPosition(void *player) {
       Vector3 out = Vector3::zero();
        void (*_Transform_GetPosition)(void *transform, Vector3 *out) = (void (*)(void *, Vector3 *))getRealOffset(oxo("0x8D6C250"));
        ...
        TransformSetPosition_fn _Transform_SetPosition = (TransformSetPosition_fn)getRealOffset(ENCRYPTOFFSET("0x8D6C328"));
    ...
    {
        void *(*_Player_GetHeadCollider)(void *players) = (void *(*)(void *))getRealOffset(oxo("0x5634F3C"));
    ...
    {
        bool (*_Physics_Raycast)(Vector3 camLocation, Vector3 headLocation, void *hit, float maxDistance, unsigned int layerMask) = (bool (*)(Vector3, Vector3, void *, float, unsigned int))getRealOffset(oxo("0x8DE4FDC"));
        return _Physics_Raycast(camLocation, headLocation, collider, 500.0f, LayerID);
    }
    ...
    static bool isVisible(void *enemy) {
        ...
        void *collider = Player_GetHeadCollider(enemy);
        void *headColliderTF = collider ? game_sdk->Component_GetTransform(collider) : nullptr;
        if (!camTF || !headColliderTF)
            return false;
        void *hitObj = NULL;
        Vector3 Camera = Transform_GetPosition(camTF);
        Vector3 Target = Transform_GetPosition(headColliderTF);
        Vector3 dir = Target - Camera;
        if (Vector3::SqrMagnitude(dir) < 0.0001f)
            return false;
        return !Physics_Raycast(Camera, Vector3::Normalized(dir), 12, &hitObj);
    }
};
    ...
    try {
        ...
        for (int i = 0; i < values.size(); i++) {
            ...
            if (Vars.IgnoreKnocked && game_sdk->get_IsDieing(player))
                continue;
            if (Vars.VisibleCheck && !Vars.skipwall && !tanghinh::isVisible(player))
                continue;
            ...
            if (Vars.aimMode == 0) { // Aim Fov
                ...
                float angle = Vector3::Angle(dir, cameraForward) * Rad2Deg;
...
void ProcessAimbot() {
    ...
    bool visOK = Vars.skipwall || !Vars.VisibleCheck || tanghinh::isVisible(closestEnemy);
    ...
    if (shouldAim && visOK) {
        ...
        game_sdk->set_aim(LocalPlayer, TargetLook, true);
    }
...
if (!Vars.Underground && !Vars.Blamyban && cameraAdjusted) {
   ...
   if (Vars.MagnetEnemy) {
    ...
    void* PlayerAttributes = *(void**)((uint64_t)local_player + 0x768);
    if (!PlayerAttributes)
        return;
...
// -------------------------------------------------------
        ...
        draw_list->AddLine(startPos, endPos, ImColor(255, 255, 255, 255), 1.2f);
                    ...
                    if (Vars.Distance) {
                        ...
                        float fontSize = 9.0f;
                        ImVec2 textSize = pixel_smol->CalcTextSizeA(fontSize, FLT_MAX, 0.0f, distancestr.c_str());
                        
                        ImVec2 distPos = ImVec2(
                            rect.Min.x + (rect.GetWidth() * 0.5f) - (textSize.x * 0.5f),
                            rect.Max.y + 3.0f
                        );

                        draw_list->AddText(pixel_smol, fontSize, distPos + ImVec2(1, 1), ImColor(0, 0, 0, 255), distancestr.c_str());
                        draw_list->AddText(pixel_smol, fontSize, distPos + ImVec2(-1, 1), ImColor(0, 0, 0, 255), distancestr.c_str());

                        draw_list->AddText(pixel_smol, fontSize, distPos, ImColor(255, 255, 255, 255), distancestr.c_str());
                    }
