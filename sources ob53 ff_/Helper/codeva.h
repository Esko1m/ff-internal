...
uint64_t q8m4n2x7(struct mach_header_64* header, uint64_t va)
{
    uint64_t rva = va;
...
void* h5k9p3w1(struct mach_header_64* header, uint64_t rva)
{
    uint64_t header_vaddr = -1;
    struct load_command* lc = (struct load_command*)((UInt64)header + sizeof(*header));
    for (int i = 0; i < header->ncmds; i++) {
...
NSMutableData* r7j2v8n4(NSString* path)
{
...
NSMutableData* w3q6m9k2(NSMutableData* macho)
{
...
bool n4z7h8p5(char* bytes, unsigned char* buffer)
{
...
uint64_t t9w3k6m2(uint64_t vaddr, char* patch)
{
    return [[[NSString stringWithUTF8String:patch] lowercaseString] hash] ^ vaddr;
}

void* j6p2n8v4(char* machoPath)
{
...
StaticInlineHookBlock* m8q4h7k3(void* base, uint64_t vaddr)
{
    ...
    StaticInlineHookBlock* hookBlock = (StaticInlineHookBlock*)((uint64_t)header + q8m4n2x7(header, data_seg->vmaddr));
...
void* z5n9w2p7(char* machoPath, uint64_t vaddr, void* replace)
{
    void* base = j6p2n8v4(machoPath);
    if(!base) {
        return NULL;
    }
    
    StaticInlineHookBlock* hookBlock = m8q4h7k3(base, vaddr);
    if(!hookBlock) {
        return NULL;
    }
...
void* v3j8q5m1(void* add, uint64_t vaddr, void* replace)
{
    if(!add) {
        return NULL;
    }
    
    StaticInlineHookBlock* hookBlock = m8q4h7k3(add, vaddr);
    if(!hookBlock) {
        return NULL;
    }
...
BOOL k7r4p9n2(char* machoPath, uint64_t vaddr, char* patch)
{
    void* base = j6p2n8v4(machoPath);
    if(!base) {
        return NO;
    ...
    StaticInlineHookBlock* hookBlock = m8q4h7k3(base, vaddr&~3);
    if(!hookBlock) {
        return NO;
    }
    
    if(hookBlock->patch_hash != t9w3k6m2(vaddr, patch)) {
        return NO;
    }
...
BOOL h2w6q8v3(char* machoPath, uint64_t vaddr, char* patch)
{
    void* base = j6p2n8v4(machoPath);
    if(!base) {
        return NO;
...
{ \
    void* result = v3j8q5m1(lib_start_addr, x, (void *) y); \
    *(void **) (&z) = (void*) result; \
}

#define InlineHookEx(lib_name, x, y, z) \
{ \
    void* result = z5n9w2p7(lib_name, x, (void *) y); \
    *(void **) (&z) = (void*) result; \
}

#define InlineHook(x, y, z) \
{ \
    void* result = z5n9w2p7(DEFAULT_MACHO_PATH, x, (void *) y); \
    *(void **) (&z) = (void*) result; \
}

#define ActiveOff(vaddr, patch) \
    k7r4p9n2(DEFAULT_MACHO_PATH, vaddr, patch)
    
#define ActiveOff2(libname, vaddr, patch) \
    k7r4p9n2(libname, vaddr, patch)
...
#define DeActiveOff(vaddr, patch) \
    h2w6q8v3(DEFAULT_MACHO_PATH, vaddr, patch)