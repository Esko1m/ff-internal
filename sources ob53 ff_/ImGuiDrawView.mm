...
- (void)saveSettings {
    ...
    [saveSetting setBool:Vars.Aimbot forKey:@"Aimbot"];
...
- (void)loadSettings {
    ...
    Vars.Aimbot = [saveSetting boolForKey:@"Aimbot"];
    ...
    Vars.isAimFov = [saveSetting boolForKey:@"isAimFov"];
    Vars.AimHitbox = [saveSetting integerForKey:@"AimHitbox"];
    Vars.aimMode = [saveSetting integerForKey:@"aimMode"];

    float savedAimFov = [saveSetting floatForKey:@"AimFov"];
    Vars.AimFov = (savedAimFov >= 30.0f) ? savedAimFov : 150.0f;
}
...
ImGui::SetNextWindowPos(ImVec2(x, y), ImGuiCond_FirstUseEver);
        ...
        if (MenDeal == true)
        {
            ImGui::Begin(ENCRYPT("DARK MODS V1.2"), &MenDeal);
...
ImGui::Columns(2, "AimBotColumns", false);
  ...
  ImGui::NextColumn();
                    ...
                    ImGui::Checkbox(ENCRYPT("Visible Only"), &Vars.VisibleCheck);
                ...
                if (ImGui::BeginTabItem(ICON_FA_COG " Setting"))
{
    ...
    ImGui::Text("Samwil King");

    if (ImGui::Button(ENCRYPT("Telegram"), ImVec2(120, 30)))

            {
                NSString *urlStr = @(ENCRYPT("https://t.me/Samwilffx"));
